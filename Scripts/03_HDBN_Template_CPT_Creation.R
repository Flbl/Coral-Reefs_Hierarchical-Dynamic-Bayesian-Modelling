...
# HIERARCHICAHL NODES GENERATION ----
      ...
      # Add hierarchy arcs (variable-specific) 
        ...
        if(!any(is.null(site), is.null(station))) {
          if (!(site %in% net$getParentIds(station))) {
            net$addArc(site, station[-grep("OBS|Trend",station)])
          }
        }
        
        # True station to obs
        if(!(station[-grep("OBS|Trend",station)] %in% net$getParentIds(station[grep("OBS|Trend",station)]))) {
          net$addArc(station[-grep("OBS|Trend",station)], station[grep("OBS|Trend",station)])
        }  
  ...
  # Create and Connect environmental drivers nodes
